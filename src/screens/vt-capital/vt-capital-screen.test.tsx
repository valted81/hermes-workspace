...
describe('VtCapitalScreen', () => {
  ...
  it('renders a plugin-scoped observability cockpit, not a generic dashboard clone', async () => {
    ...
    expect(container.textContent).toContain('Ordine duplicato già aperto')
    ...
    expect(container.textContent).toContain('Segnali → Concilium → Guardian → paper/demo')
    expect(container.textContent).toContain('Order proposal0')
    expect(container.textContent).toContain(
      'Serve più storico forward prima di procedere',
    )
    ...
    expect(container.textContent).toContain('Serve più evidenza forward')
    ...
    expect(container.textContent).toContain('Max Dca Underwater Exceeded')
    ...
    expect(container.textContent).toContain('Concilium Watch Risk Gate Passed')
    expect(container.textContent).toContain('Non promosso automaticamente')
    ...
    expect(container.textContent).toContain('Broker disattivato')

...
import {
  ...
  readRuntimeManualApproval,
  readRuntimeManualApprovalActions,
...
describe('VT Capital backtest data API summary', () => {
  ...
  it('reads the forward observe queue without enabling execution', () => {
    const summary = readForwardTestQueue()
  ...
  it('reads runtime manual approval requests without promoting paper', () => {
    const summary = readRuntimeManualApproval()

    expect(summary).toHaveProperty('requestCount')
    expect(summary).toHaveProperty('blockedCount')
  ...
  it('reads runtime manual approval actions without execution side effects', () => {
    const summary = readRuntimeManualApprovalActions()

...
const VT_RUNTIME_CONCILIUM_PATH = path.join(
  VT_REPO_DIR,
  'data/runtime/concilium-decisions.json',
)
const VT_RUNTIME_CONCILIUM_LOG_PATH = path.join(
  VT_REPO_DIR,
  'data/runtime/concilium-decisions.jsonl',
)
const VT_RUNTIME_ORDER_PROPOSALS_PATH = path.join(
  VT_REPO_DIR,
  'data/runtime/order-proposals.json',
)
const VT_RUNTIME_ORDER_PROPOSALS_LOG_PATH = path.join(
  VT_REPO_DIR,
...
const VT_RUNTIME_MANUAL_APPROVAL_ACTIONS_PATH = path.join(
  VT_REPO_DIR,
  'data/runtime/manual-approval-actions.json',
)
const VT_RUNTIME_MANUAL_APPROVAL_ACTIONS_LOG_PATH = path.join(
  VT_REPO_DIR,
  'data/runtime/manual-approval-actions.jsonl',
)
const VT_RUNTIME_SIGNALS_PATH = path.join(
  VT_REPO_DIR,
  'data/runtime/signals.json',
)
const VT_RUNTIME_SIGNALS_LOG_PATH = path.join(
  VT_REPO_DIR,
  'data/runtime/signals.jsonl',
)
const VT_RUNTIME_PAPER_POSITIONS_PATH = path.join(
  VT_REPO_DIR,
  'data/runtime/paper-positions.json',
)
const VT_RUNTIME_PAPER_POSITIONS_LOG_PATH = path.join(
  VT_REPO_DIR,
  'data/runtime/paper-positions.jsonl',
)
const VT_RUNTIME_DEMO_BRIDGE_PATH = path.join(
  VT_REPO_DIR,
  'data/runtime/demo-broker-bridge.json',
)
const VT_RUNTIME_DEMO_BRIDGE_LOG_PATH = path.join(
  VT_REPO_DIR,
...
const VT_RUNTIME_DEMO_POSITIONS_PATH = path.join(
  VT_REPO_DIR,
  'data/runtime/demo-positions.json',
)
const VT_RUNTIME_DEMO_POSITIONS_LOG_PATH = path.join(
  VT_REPO_DIR,
...
const VT_RUNTIME_PORTFOLIO_STATUS_LOG_PATH = path.join(
  VT_REPO_DIR,
  'data/runtime/portfolio-status.jsonl',
)
const VT_RUNTIME_OPERATIONAL_JOURNAL_PATH = path.join(
  VT_REPO_DIR,
  'data/runtime/operational-journal.json',
)
const VT_RUNTIME_OPERATIONAL_JOURNAL_LOG_PATH = path.join(
  VT_REPO_DIR,
  'data/runtime/operational-journal.jsonl',
)
...
export function readRuntimeConcilium(): JsonRecord {
  const stat = safeStat(VT_RUNTIME_CONCILIUM_PATH)
  const logStat = safeStat(VT_RUNTIME_CONCILIUM_LOG_PATH)
  const raw = readJsonFile(VT_RUNTIME_CONCILIUM_PATH)
  ...
  return {
    ...
    logExists: Boolean(logStat),
    logUpdatedAt: logStat?.mtimeMs ?? null,
    logEventCount: countJsonlRecords(VT_RUNTIME_CONCILIUM_LOG_PATH),
    generatedAt:
      typeof raw?.generated_at === 'string' ? raw.generated_at : null,
    mode:
      typeof raw?.mode === 'string'
        ? raw.mode
        : 'runtime_concilium_observe_only',
    decisionCount:
      typeof raw?.decision_count === 'number'
        ? raw.decision_count
        : decisions.length,
    decisionCounts:
      raw?.decision_counts && typeof raw.decision_counts === 'object'
        ? raw.decision_counts
        : {},
    agentProfiles: Array.isArray(raw?.agent_profiles) ? raw.agent_profiles : [],
    decisions,
    safety: {
      observeOnly: safety.observe_only !== false,
      executionEnabled: safety.execution_enabled === true,
...
export function readRuntimeOrderProposals(): JsonRecord {
  const stat = safeStat(VT_RUNTIME_ORDER_PROPOSALS_PATH)
  const logStat = safeStat(VT_RUNTIME_ORDER_PROPOSALS_LOG_PATH)
  const raw = readJsonFile(VT_RUNTIME_ORDER_PROPOSALS_PATH)
  ...
  return {
    ...
    logExists: Boolean(logStat),
    logUpdatedAt: logStat?.mtimeMs ?? null,
    logEventCount: countJsonlRecords(VT_RUNTIME_ORDER_PROPOSALS_LOG_PATH),
    generatedAt:
      typeof raw?.generated_at === 'string' ? raw.generated_at : null,
    mode:
      typeof raw?.mode === 'string'
        ? raw.mode
        : 'runtime_order_proposals_observe_only',
    proposalCount:
      typeof raw?.proposal_count === 'number'
        ? raw.proposal_count
        : proposals.length,
    heldCount:
      typeof raw?.held_count === 'number'
        ? raw.held_count
        : heldDecisions.length,
    proposals,
    heldDecisions,
    safety: {
      observeOnly: safety.observe_only !== false,
...
export function readRuntimeSignals(): JsonRecord {
  const stat = safeStat(VT_RUNTIME_SIGNALS_PATH)
  const logStat = safeStat(VT_RUNTIME_SIGNALS_LOG_PATH)
  const raw = readJsonFile(VT_RUNTIME_SIGNALS_PATH)
  const signals = Array.isArray(raw?.signals) ? raw.signals : []
  const actionableSignals = Array.isArray(raw?.actionable_signals)
    ? raw.actionable_signals
    : []
  const safety =
    raw?.safety && typeof raw.safety === 'object' && !Array.isArray(raw.safety)
      ? (raw.safety as JsonRecord)
  ...
  return {
    ...
    logExists: Boolean(logStat),
    logUpdatedAt: logStat?.mtimeMs ?? null,
    logEventCount: countJsonlRecords(VT_RUNTIME_SIGNALS_LOG_PATH),
    generatedAt:
      typeof raw?.generated_at === 'string' ? raw.generated_at : null,
    mode:
      typeof raw?.mode === 'string'
        ? raw.mode
        : 'runtime_signal_scanner_observe_only',
    desk: typeof raw?.desk === 'string' ? raw.desk : null,
    signalCount:
      typeof raw?.signal_count === 'number' ? raw.signal_count : signals.length,
    actionableCount:
      typeof raw?.actionable_count === 'number'
        ? raw.actionable_count
...
export function readRuntimePaperPositions(): JsonRecord {
  const stat = safeStat(VT_RUNTIME_PAPER_POSITIONS_PATH)
  const logStat = safeStat(VT_RUNTIME_PAPER_POSITIONS_LOG_PATH)
  const raw = readJsonFile(VT_RUNTIME_PAPER_POSITIONS_PATH)
  ...
  return {
    ...
    logExists: Boolean(logStat),
    logUpdatedAt: logStat?.mtimeMs ?? null,
    logEventCount: countJsonlRecords(VT_RUNTIME_PAPER_POSITIONS_LOG_PATH),
    generatedAt:
      typeof raw?.generated_at === 'string' ? raw.generated_at : null,
    mode:
      typeof raw?.mode === 'string'
        ? raw.mode
        : 'runtime_paper_positions_observe_only',
    positionCount:
      typeof raw?.position_count === 'number'
        ? raw.position_count
        : positions.length,
    ...
    events,
    safety: {
      paperSimulatedOnly: safety.paper_simulated_only !== false,
      brokerCallsAllowed: safety.broker_calls_allowed === true,
      executionEnabled: safety.execution_enabled === true,
      liveTradingEnabled: safety.live_trading_enabled === true,
    },
...
export function readRuntimeDemoBridge(): JsonRecord {
  const stat = safeStat(VT_RUNTIME_DEMO_BRIDGE_PATH)
  const logStat = safeStat(VT_RUNTIME_DEMO_BRIDGE_LOG_PATH)
  const raw = readJsonFile(VT_RUNTIME_DEMO_BRIDGE_PATH)
  ...
  return {
    ...
    logExists: Boolean(logStat),
    logUpdatedAt: logStat?.mtimeMs ?? null,
    logEventCount: countJsonlRecords(VT_RUNTIME_DEMO_BRIDGE_LOG_PATH),
    generatedAt:
      typeof raw?.generated_at === 'string' ? raw.generated_at : null,
    mode:
      typeof raw?.mode === 'string'
        ? raw.mode
        : 'runtime_demo_broker_bridge_controlled',
    brokerMode: typeof raw?.broker_mode === 'string' ? raw.broker_mode : null,
    orderCount:
      typeof raw?.order_count === 'number' ? raw.order_count : orders.length,
    submittedEventCount:
      typeof raw?.submitted_event_count === 'number'
        ? raw.submitted_event_count
        : events.length,
    skippedCount:
    ...
    safety: {
      ...
      maxNotionalUsdt:
        typeof safety.max_notional_usdt === 'number'
          ? safety.max_notional_usdt
          : null,
    },
  }
}
...
export function readRuntimeDemoPositions(): JsonRecord {
  const stat = safeStat(VT_RUNTIME_DEMO_POSITIONS_PATH)
  const logStat = safeStat(VT_RUNTIME_DEMO_POSITIONS_LOG_PATH)
  const raw = readJsonFile(VT_RUNTIME_DEMO_POSITIONS_PATH)
  const positions = Array.isArray(raw?.positions) ? raw.positions : []
  const ignoredOrders = Array.isArray(raw?.ignored_orders) ? raw.ignored_orders : []
  const events = Array.isArray(raw?.events) ? raw.events : []
  const safety =
    raw?.safety && typeof raw.safety === 'object' && !Array.isArray(raw.safety)
      ? (raw.safety as JsonRecord)
  ...
  return {
    ...
    logExists: Boolean(logStat),
    logUpdatedAt: logStat?.mtimeMs ?? null,
    logEventCount: countJsonlRecords(VT_RUNTIME_DEMO_POSITIONS_LOG_PATH),
    generatedAt:
      typeof raw?.generated_at === 'string' ? raw.generated_at : null,
    mode:
      typeof raw?.mode === 'string'
        ? raw.mode
        : 'runtime_demo_positions_from_broker_monitor',
    positionCount:
    ...
    openOrderCount:
      typeof raw?.open_order_count === 'number' ? raw.open_order_count : 0,
    canceledOrderCount:
      typeof raw?.canceled_order_count === 'number' ? raw.canceled_order_count : 0,
    ignoredOrderCount:
      typeof raw?.ignored_order_count === 'number' ? raw.ignored_order_count : ignoredOrders.length,
    positions,
    ignoredOrders,
    events,
    safety: {
...
export function readRuntimePortfolioStatus(): JsonRecord {
  const stat = safeStat(VT_RUNTIME_PORTFOLIO_STATUS_PATH)
  const logStat = safeStat(VT_RUNTIME_PORTFOLIO_STATUS_LOG_PATH)
  const raw = readJsonFile(VT_RUNTIME_PORTFOLIO_STATUS_PATH)
  const paper =
    raw?.paper && typeof raw.paper === 'object' && !Array.isArray(raw.paper)
      ? (raw.paper as JsonRecord)
      : {}
  const demoBroker =
    raw?.demoBroker &&
    typeof raw.demoBroker === 'object' &&
    !Array.isArray(raw.demoBroker)
      ? (raw.demoBroker as JsonRecord)
      : {}
  const totals =
  ...
  return {
    ...
    logExists: Boolean(logStat),
    logUpdatedAt: logStat?.mtimeMs ?? null,
    logEventCount: countJsonlRecords(VT_RUNTIME_PORTFOLIO_STATUS_LOG_PATH),
    generatedAt:
      typeof raw?.generated_at === 'string' ? raw.generated_at : null,
    mode:
      typeof raw?.mode === 'string'
        ? raw.mode
        : 'runtime_portfolio_status_read_only',
    paper,
    demoBroker,
    totals,
    reconciliation,
...
export function readRuntimeOperationalJournal(): JsonRecord {
  const stat = safeStat(VT_RUNTIME_OPERATIONAL_JOURNAL_PATH)
  const logStat = safeStat(VT_RUNTIME_OPERATIONAL_JOURNAL_LOG_PATH)
  const raw = readJsonFile(VT_RUNTIME_OPERATIONAL_JOURNAL_PATH)
  const events = Array.isArray(raw?.events) ? raw.events : []
  const countsByType =
    raw?.counts_by_type && typeof raw.counts_by_type === 'object' && !Array.isArray(raw.counts_by_type)
      ? (raw.counts_by_type as JsonRecord)
      : {}
  const countsBySeverity =
    raw?.counts_by_severity &&
    typeof raw.counts_by_severity === 'object' &&
    !Array.isArray(raw.counts_by_severity)
      ? (raw.counts_by_severity as JsonRecord)
      : {}
  const safety =
    raw?.safety && typeof raw.safety === 'object' && !Array.isArray(raw.safety)
      ? (raw.safety as JsonRecord)
  ...
  return {
    ...
    logExists: Boolean(logStat),
    logUpdatedAt: logStat?.mtimeMs ?? null,
    logEventCount: countJsonlRecords(VT_RUNTIME_OPERATIONAL_JOURNAL_LOG_PATH),
    generatedAt:
      typeof raw?.generated_at === 'string' ? raw.generated_at : null,
    mode:
      typeof raw?.mode === 'string'
        ? raw.mode
        : 'runtime_operational_journal_read_only',
    eventCount: typeof raw?.event_count === 'number' ? raw.event_count : events.length,
    countsByType,
    countsBySeverity,
    events,
    safety: {
...
export function readRuntimeManualApprovalActions(): JsonRecord {
  const stat = safeStat(VT_RUNTIME_MANUAL_APPROVAL_ACTIONS_PATH)
  const logStat = safeStat(VT_RUNTIME_MANUAL_APPROVAL_ACTIONS_LOG_PATH)
  const raw = readJsonFile(VT_RUNTIME_MANUAL_APPROVAL_ACTIONS_PATH)
  const approvals = Array.isArray(raw?.approvals) ? raw.approvals : []
  const blockedRequests = Array.isArray(raw?.blocked_requests)
    ? raw.blocked_requests
    : []
  const safety =
    raw?.safety && typeof raw.safety === 'object' && !Array.isArray(raw.safety)
      ? (raw.safety as JsonRecord)
  ...
  return {
    ...
    logExists: Boolean(logStat),
    logUpdatedAt: logStat?.mtimeMs ?? null,
    logEventCount: countJsonlRecords(
      VT_RUNTIME_MANUAL_APPROVAL_ACTIONS_LOG_PATH,
    ),
    generatedAt:
      typeof raw?.generated_at === 'string' ? raw.generated_at : null,
    mode:
      typeof raw?.mode === 'string'
        ? raw.mode
        : 'runtime_manual_approval_actions_protected',
    requestCount:
      typeof raw?.request_count === 'number' ? raw.request_count : approvals.length + blockedRequests.length,
    approvedCount:
      typeof raw?.approved_count === 'number' ? raw.approved_count : approvals.length,
    blockedCount:
      typeof raw?.blocked_count === 'number' ? raw.blocked_count : blockedRequests.length,
    approvals,
    blockedRequests,
    safety: {
      manualApprovalRequired: safety.manual_approval_required !== false,
      ...
      GET: ({ request }) => {
        ...
        return json({
          ...
          paths: {
            ...
            runtimeConcilium: VT_RUNTIME_CONCILIUM_PATH,
            ...
            runtimeManualApprovalActions:
              VT_RUNTIME_MANUAL_APPROVAL_ACTIONS_PATH,
            runtimeManualApprovalActionsLog:
              VT_RUNTIME_MANUAL_APPROVAL_ACTIONS_LOG_PATH,
            runtimeSignals: VT_RUNTIME_SIGNALS_PATH,
            runtimeSignalsLog: VT_RUNTIME_SIGNALS_LOG_PATH,
            runtimePaperPositions: VT_RUNTIME_PAPER_POSITIONS_PATH,
            ...
            runtimeOperationalJournalLog: VT_RUNTIME_OPERATIONAL_JOURNAL_LOG_PATH,
            home: os.homedir(),
          },
          marketBias: {
          ...
          runtimeManualApprovalActions: readRuntimeManualApprovalActions(),
